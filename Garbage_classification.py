...
def token():
    ...
    request_url = 'https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=' + \
        AK+'&client_secret='+SK
...
def baiduace(access_token):
    ...
    try:
        ...
        data = {garbage1: percent1, garbage2: percent2,
                garbage3: percent3, garbage4: percent4, garbage5: percent5}
...
def classify(select_data):
    for i in select_data:
        try:
            ...
            )['data']['description']['Concept']  # 垃圾基本概念
            including = response.json(
            )['data']['description']['Including']  # 垃圾主要包括
            release_requirement = response.json(
            )['data']['description']['Release_requirement']  # 投放要求
            echo = '垃圾类别：'+type+'\n垃圾基本概念：'+concept+'\n垃圾主要包括：' + \
